...
public class SettingsActivity extends SlideoutMenuActivity {
  ...
  private void notifyDataSetChanged() {
    ...
    if (mOpenInBrowser) {
      browserCheckbox.setImageResource(R.drawable.chkbox_chkd_dark);
    } else {
      browserCheckbox.setImageResource(R.drawable.chkbox_default_dark);
    }

    ImageView bugReportCheckbox = (ImageView) mBugReportsButton.findViewById(R.id.img_checkbox);
    if (mSubmitBugReports) {
      bugReportCheckbox.setImageResource(R.drawable.chkbox_chkd_dark);
    } else {
      bugReportCheckbox.setImageResource(R.drawable.chkbox_default_dark);
    }
  }
}
