...
/**
 * The MainActivity is the host for the StoryFragment, CommentsFragment, and WebFragment. <p/> It's also responsible
 * for: - deciding whether to show a two-pane or one-pane layout - managing the ActionBar tabs for Comments & Web
 * Fragment - managing the SharePopup - managing the Search and TextSettings in the overflow menu - implementing the
 * TabletLayout interface - managing communications between fragments by implementing any Callback interfaces it's child
 * fragments require *
 */
public class MainActivity extends SlideoutMenuActivity implements SharePopupInterface, TabletLayout,
  ...
  public static final String IS_COMMENTS_ACTIVITY = NAME + ".isCommentsActivity";
  public static final String STORY = NAME + ".story";
  public static final String INITIAL_TAB_POSITION = NAME + ".initialTabPosition";
  public static final String PAGE = NAME + ".page";
  ...
  private OnPageChangeListener mPageListener = new ViewPager.OnPageChangeListener() {
    ...
    public void onPageSelected(int position) {
      ...
      if (mCurrentTabPosition == COMMENTS_POS) {
        mCommentsFragment.cleanupActionBar(context, controller);
      }
      if (mCurrentTabPosition == ARTICLE_POS) {
        mWebFragment.cleanupActionBar(context, controller);
      }
      if (position == COMMENTS_POS) {
        mCommentsFragment.setupActionBar(context, controller);
      }
      if (position == ARTICLE_POS) {
        mWebFragment.setupActionBar(context, controller);
      }
  ...
  /**
   * Find the ViewPager for Comments & WebFragment, attach the adapter, create the fragments if necessary, and create
   * the tabs for switching between them.
   */
  ...
  private void setupCommentsFragment(Bundle savedInstanceState) {
    ...
    if (mStory != null) {
      mCommentsFragment.setStory(mStory);
    }
  ...
  private void setupWebFragment(Bundle savedInstanceState) {
    ...
    if (mStory != null) {
      mWebFragment.setUrl(mStory.url);
    }
  ...
  private void setupSearchAndTextSettingsOverflow() {
    ...
    // setup search box
    if (mSearch == null) {
      mSearch = new SearchOverflow(this, this);
    }
    controller.addOverflowView(mSearch);
    // setup text settings
    if (mTextSettings == null) {
      mTextSettings = new TextSettingsOverflow(this, this);
    }
  ...
  private void retrieveBundles(Bundle savedInstanceState, Bundle extras) {
    ...
    // default values
    if (mInitialTabPosition == null) {
      mInitialTabPosition = CommentsTab.COMMENTS;
    }
    if (mPage == null) {
      mPage = Page.FRONT;
    }
  ...
  /**
   * Try to retrieve fragments by tag, will be null if not found (and thus will get created by their respective "setup"
   * methods
   */
  ...
  public void showCommentsForStory(Story story, CommentsTab initialTab) {
    if (mActivityType == ActivityType.TABLET) {
      ...
      if (!mIsJobsPost) {
        mCommentsFragment.loadNewStory(story);
      }
      if (StringUtils.isNotBlank(mStory.url)) {
        mWebFragment.loadNewUrl(mStory.url);
      }
  ...
  public Story getActiveStory() {
    if (mCommentsFragment != null) {
      return mCommentsFragment.getStory();
    } else {
      return null;
    }
  }
   ...
   */
  public static void startCommentsActivity(Context context, Page page, int storyId, String url,
                                           CommentsTab initialTab) {
